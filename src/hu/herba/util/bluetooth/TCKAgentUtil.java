...
public class TCKAgentUtil {
	...
	private static final int NO_SECURITY = ServiceRecord.NOAUTHENTICATE_NOENCRYPT;
	...
	public static int getServiceClass(final String btAddress, final int timeout) {
		LocalDevice device;
		DiscoveryAgent da;
	...
	/*
	 * Starts an inquiry to find the remoteDevice with the address passed in as a parameter.
	 */
	...
	/*
	 * This method starts an serviceSearch and returns the array of serviceRecords it finds. It takes a bluetooth address and a UUID array as the input
	 * parameters.
	 */
	public static ServiceRecord[] getServiceRecords(final String btAddress, final UUID[] uArr) {
		DiscoveryListenerImpl listen;
		...
		try {
			...
			if (dev == null) {
				System.out.println("TCKAgentUtil : ERROR. Unable to" + " retrieve Remote BT Device.");
				return null;
			}
			...
			System.out.println("TCKAgentUtil : ERROR. Unable to " + "retrieve Remote Service Records.");
			return null;
		}
	...
	/*
	 * Utility method that returns one of the service records registered by the BluetoothTCKAgent on the device with Bluetooth address btAddress. The service
	 * record returned depends on the uuid argument, which is the 16-bit UUID defined in the Bluetooth assigned numbers for one of the protocols L2CAP, RFCOM or
	 * OBEX. The service record returned describes how to connect to the agent using this protocol. Returns null if the service record is not found or if uuid
	 * is not one of 0x0003, 0x0100, or 0x0008.
	 */

	public static ServiceRecord getServiceRecord(final String btAddress, final int uuid) {
	...
	/*
	 * Utility method that returns the connection URL for one of the services offered by the BluetoothTCKAgent on the device btAddress. The uuid is one of the
	 * three 16-bit UUIDs recognized by the getServiceRecord(btAddress, uuid) method. Returns null if the the service record can't be found or if
	 * getConnectionURL() cannot create a connection string for the service record.
	 */

	public static String getURL(final String btAddress, final int uuid) {
